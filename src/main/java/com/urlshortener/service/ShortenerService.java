...
import com.urlshortener.util.HttpUtil;
import com.urlshortener.util.IdGeneratorServiceCaller;
...
public class ShortenerService {
    ...
    private final IdGeneratorServiceCaller idGeneratorServiceCaller;
    ...
    public ShortenerService(CacheRepo cacheRepo, URLRepo repo, IdGeneratorServiceCaller idGeneratorServiceCaller) {
        this.cacheRepo = cacheRepo;
        this.repo = repo;
        this.supplyFromCache = cacheRepo::getUrlFromCache;
        this.supplyFromDB = s -> repo.findByShortUrl(s).map(URLInfo::getShortUrl);
        this.idGeneratorServiceCaller = idGeneratorServiceCaller;
    }
    ...
    private Optional<String> processBusinessLogic(String urlToProcess) {
        ...
        var id = idGeneratorServiceCaller.getIdFromIdService()
                .orElseGet(() -> HashUtils.sha256Hasher(uri.toString())
